...
module.exports =
  class AMQPSenecaListener {
    constructor(seneca, transport, options) {
      ...
      this.messageThrottling = require('./message-throttling').initializeMessageThrottling(options.messageThrottling || {});
    }
    ...
    consume() {
      return (message) => {
        ...
        // Requeue the message if this service has reached its limit
        if (this.messageThrottling.enabled) {
          if (this.messageThrottling.numberOfHandledMessages === this.messageThrottling.maximumMessages) {
            return this.transport.channel.nack(message, false, true);
          }
          this.messageThrottling.incrementNumberOfHandledMessages();
        }
